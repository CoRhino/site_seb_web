...
- [ ] **Handles/URLs manquants pour `contact.html`** : LinkedIn, Facebook, TikTok, Tinder.
      (Le footer de toutes les pages utilise déjà les 3 « meilleurs » : Bandcamp `corhino.bandcamp.com`, YouTube `@CoRhino`, Instagram `@CoRhino`.)
- [ ] **Arbitrage email** : tu as demandé de retirer toute adresse courriel du site (fait). Le PLAN prévoyait un forwarding `contact@ananasday.com` — **devenu sans objet ?** À confirmer.
